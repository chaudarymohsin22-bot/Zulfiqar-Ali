...
const GENTS_CATEGORIES = [
  ...
  'Linen',
  'Boski',
  'Shalwar Qameez Fabric',
  'Premium Suiting'
];

export const GentsPage: React.FC = () => {
  ...
  // Filter only Gents products
  const gentsProducts = useMemo(() => {
    return products.filter((p) => p.gender === 'Gents');
  }, [products]);

  const availableFabrics = useMemo(() => {
    const set = new Set<string>();
    gentsProducts.forEach((p) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div className="bg-[#0B3B2C] text-white rounded-2xl p-6 sm:p-10 mb-8 border-b-4 border-[#C29B38]">
        <div className="max-w-3xl">
          <span className="text-xs font-bold uppercase tracking-widest text-[#E5C365] block mb-1">
            Men's Fabric Gallery
          </span>
          <h1 className="font-cinzel text-3xl sm:text-4xl font-bold tracking-tight mb-2">
            Gents Unstitched Collection
          </h1>
          <p className="font-urdu text-xl text-[#E5C365] mb-3">
            مردانہ شاندار اور پروقار ان سلا فیبرک کلیکشن
          </p>
          <p className="text-sm text-[#FAF7F2]/80 leading-relaxed font-light">
            Luxury wash & wear, premium 100% pure combed cotton latha, authentic Chinese weave Boski, warm Kamalia khaddar, and executive suiting fabrics crafted for custom shalwar qameez and waistcoats.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-[#E8DFD3] mb-8 shadow-2xs">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search gents fabrics by name, code (e.g. MCH-G-201), color..."
              value={searchKeyword}
        ...
        <div className="mt-3 pt-3 border-t border-[#E8DFD3]/60 flex items-center justify-between text-xs text-gray-500">
          <span>Showing {filteredProducts.length} of {gentsProducts.length} Gents unstitched fabrics</span>
      ...
      {filteredProducts.length === 0 ? (
        <div className="bg-white rounded-2xl border border-[#E8DFD3] p-12 text-center max-w-lg mx-auto">
          <p className="font-cinzel text-lg font-bold text-[#0B3B2C] mb-2">No Gents Fabrics Found</p>
